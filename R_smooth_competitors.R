...
# ---- Directories ----
clustering_methods_1d <- c("smooth_funFEM", "smooth_fda_usc", "smooth_fdacluster") # 
space <- "1d"
...
for (method in clustering_methods_1d) {
  for (dist in distributions) {
    ...
    for (n in 0:(params$N - 1)) {
      # Read data
      data_file <- file.path(data_dir, space, "smooth", dist, paste0(dist, "_", n, ".csv"))
...
clustering_methods_2d <- c("smooth_tensor_pca") #
space <- "2d"
...
clustering_methods_3d <- c("smooth_tensor_pca") #
space <- "3d"
distributions <- c("gauss") #"wave", 
