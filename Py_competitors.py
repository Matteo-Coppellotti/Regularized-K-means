...
for dims in ["1d"]:
    for curve in curve_types:
        ...
        out_path = os.path.join(output_dir, dims, "scikit", curve)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        os.makedirs(out_path, exist_ok=True)

        for i in range(N):
            Y = pd.read_csv(os.path.join(data_dir, dims, curve, f"{curve}_{i}.csv"), header=None).values
            ...
            # Save labels
            memb_path = os.path.join(output_dir, dims, "scikit", curve, "memberships.csv")
            row_df = pd.DataFrame([model.labels_])
            row_df.to_csv(memb_path, mode='a', index=False, header=False)

# 1D flatten + PCA + KMeans
for dims in ["1d"]:
    for curve in curve_types:
        out_path = os.path.join(output_dir, dims, "scikit_pca", curve)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        os.makedirs(out_path, exist_ok=True)
        ...
        for i in range(N):
            ...
            data_path = os.path.join(data_dir, dims, curve, f"{curve}_{i}.csv")
...
for dims in ["1d"]:
    for curve in curve_types:
        out_path = os.path.join(output_dir, dims, "scikit_fpca", curve)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        os.makedirs(out_path, exist_ok=True)
...
for dims in ["2d"]:
    for curve in curve_types:
        out_path = os.path.join(output_dir, dims, "scikit_hog", curve)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        os.makedirs(out_path, exist_ok=True)
...
for dims in ["2d"]:
    for curve in curve_types:
        out_path = os.path.join(output_dir, dims, "geomstats", curve)
        if os.path.exists(out_path):
            shutil.rmtree(out_path)
        os.makedirs(out_path, exist_ok=True)
